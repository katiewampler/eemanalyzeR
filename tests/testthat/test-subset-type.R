test_that("sample selection works", {
  ...
  #use negate = TRUE to get non tea samples
    ...
    expect_equal(get_sample_info(nontea, "sample"),
                 c("B1S1ExampleBlankBEM", "B1S1ExampleBlankSEM","B1S2ExampleTeaStdBEM","B1S3ExampleSampleBEM", "B1S3ExampleSampleSEM"))

  #check that multiple doesn't fail
    blk <- subset_type(eem, c("iblank", "sblank"))
    expect_equal(get_sample_info(blk, "sample"), c("B1S1ExampleBlankBEM", "B1S1ExampleBlankSEM",  "B1S2ExampleTeaStdBEM", "B1S3ExampleSampleBEM"))
