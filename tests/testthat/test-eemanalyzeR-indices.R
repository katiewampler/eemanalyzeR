...
test_that("absorbance indices are correct", {
  ...
  #SUVA Values (stardom seems to add an extra log(10) that shouldn't be there and multiples by 100 already)
    ...
    expect_equal(index$value[index$index == "SUVA254"], stardom_index$a254/doc_abs/log(10), tolerance=1e-5)
    expect_equal(index$value[index$index == "SUVA280"], stardom_index$a280/doc_abs/log(10), tolerance=1e-5)
    expect_equal(index$value[index$index == "SVA412"], stardom_index$a412/doc_abs/log(10), tolerance=1e-5)

  #spectral slopes
    expect_equal(index$value[index$index == "S275_295"], stardom_index$S275_295, tolerance=1e-5)
    expect_equal(index$value[index$index == "S350_400"], stardom_index$S350_400, tolerance=1e-5)
    expect_equal(index$value[index$index == "SR"], stardom_index$SR, tolerance=1e-5)

  #ratios
    expect_equal(index$value[index$index == "E2_E3"], stardom_index$E2_E3, tolerance=1e-5)
    expect_equal(index$value[index$index == "E4_E6"], stardom_index$E4_E6, tolerance=1e-5)
})
