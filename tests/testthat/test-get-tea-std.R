test_that("eem tea std is calculated", {
  ...
  expect_warning(expect_warning(eem_tea <- get_tea_std(file.path(system.file("extdata", package = "eemanalyzeR"), "long-term-tea"),
...
test_that("abs tea is calculated", {
  ...
  #ensure it returns warning with example data
  expect_warning(abs_tea <- get_tea_std(file.path(system.file("extdata", package = "eemanalyzeR"), "long-term-tea"),
                                        meta_name="longtermteastd-metadata.csv", pattern = "longterm-tea",
