test_that("absorbance gets interpolated", {
  ...
  abslist <- abs_interp(example_absorbance)
...
test_that("errors are given", {
  ...
  expect_error(abs_interp(example_absorbance, type="wrong"), "type %in%")
...
test_that("different interpolation methods work", {
  ...
  abslist1 <- abs_interp(example_absorbance[[1]], type="spline")
  abslist2 <- abs_interp(example_absorbance[[1]], type="linear")
