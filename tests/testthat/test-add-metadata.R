#testing loaded metadata
  ...
  meta$sample_id[5] <- "missing_sample"
  ...
  test_that("metadata is added", {
    ...
    expect_equal(abs_names[[1]], c("file","sample", "n", "data","location","sample_id","dilution","analysis_date", "sample_name","doc_mgL","notes"))
    ...
    expect_equal(eem_names[[1]], c("file","sample", "x", "ex", "em", "location","sample_id","dilution",
                                   "integration_time_s", "raman_area_1s", "analysis_date", "sample_name","doc_mgL","notes"))

    abs_aug <- add_metadata(metadata, example_abs)
    eem_aug <- add_metadata(metadata, example_eems)

    expect_equal(get_sample_info(eem_aug, "sample_id"), rep(c("ExampleBlank", "ExampleTeaStd", "ExampleSample", "ManualExampleTea"), each=2))
  ...
  test_that("Metadata adds sample type with and without sample_type in metadata", {
    ...
    expect_warning(eems_w_meta_guessed <- add_metadata(meta_without_sample_type, example_eems),
                   "No sample_type in Metadata. Guessing sample_types by pattern matching sample_id")
  ...
  test_that("metadata add works with names containing each other", {
   #make contained names
    ...
    meta$sample_id[2] <- "ExampleSample1"
