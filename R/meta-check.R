...
meta_check <- function(meta){
  #identify if old meta columns data_identifier and description exist and rename
  meta <- meta %>% rename(any_of(c(sample_name = "description", sample_id = "data_identifier")))

  #check required columns are there
  req_cols <- c("sample_id", "replicate_no", "integration_time_s", "RSU_area_1s",
                "dilution")
  ...
  #ensure sample_id isn't missing data
  if(any(is.na(meta$sample_id) | is.character(meta$sample_id) == F)){
    stop("metadata missing sample_id for one or more samples, \nsee help(eemanalyzeR::metadata) for more info")
  }

  #make sure there aren't any duplicated samples
  unique_ID <- paste(meta$sample_id, meta$replicate_no, meta$integration_time_s, sep="_")
  if(any(duplicated(unique_ID))){
    stop("duplicate samples found with the same sample_id, replicate_no, and integration time:\n", paste(unique(unique_ID[duplicated(unique_ID)]), sep="\n"), "\nif they are indeed duplicates, please use 'replicate_no' to note this by using 1,2,3...")
  }
