...
To maximize flexibility in calculating indices, the `get_indices()`
...
1.  **The function needs to have the following arguments:**
    ...
    d.  `qaqc_dir`: the file path to the mdl files generated with
        `create_mdl()`
...
-   `value`: the value of the index, also accepts flag values (see
    details below)
...
`get_absorbance()` function can be used. This function can additionally
provide specific absorbance (like SUVA 254).
...
data: `get_fluorescence()`. Here, you can specify a range of excitation
and/or emission wavelengths. If stat is set to `sum` the sum of
fluorescence will be returned, otherwise if `max` is used the maximum
fluorescence across that range will be returned.
...
`get_abs_slope()` which will convert the absorbance to absorption,
interpolate the data if needed, and return the spectral slope within a
wavelength range.
...
While the `get_indices()` function includes some QA/QC checks (negative
numbers, missing values, below noise, outside expected range, etc.),
there are some checks that are easier to do when calculating the
indices, or you may wish to include custom flags or checks. To pass
flags from your index function to the output, they need to get put in
the `value` column of the index data.frames.
...
First is the `get_ratios()` function. This can be helpful when
calculating things like peak ratios, where a missing value will result
in an `NA` or a value of 0 in the denominator will result in an infinite
value.
...
`flag_missing()`. This is useful to check when calculating indices to
flag why certain indices might be returning `NA` values.
...
this can be done with: `check_eem_mdl()` and `check_abs_mdl()`. This is
useful to ensure that the blanks are actually blank and ratio values
that are returned are based on "real" data.
...
data and an MDL flag) you can use the `.combine_flags()` function to
nicely combine the flags.
...
formatted in a specific way to input into the `get_indices()` function.
To do this we can use the `format_index()` function.
...
We can use the `get_fluorescence()` helper function to calculate the
fluorescence for the numerator and denominator of the ratio. Then we can
use the `get_ratios()` helper function to get the ratio. For a single
index this would look like:
...
the MDL, since we have two discrete values we can use `.combine_flags()`
to combine the returned MDL flags. Lastly, we'll use the
`format_index()` function to make sure everything is correctly
formatted.
...
still run. We can use the `get_qaqc()` function
...
Now let's test our custom function with the `get_indices()` function.
...
We can use the `get_absorbance()` helper function to calculate the
absorbance at 254 and 420. For a single wavelength that looks like:
...
correctly. We'll use `base::do.call()` to bind all the lists of indices
together.
...
Next lets get the ratios, we can still use the `get_absorbance()` helper
function to get the absorbance at the wavelengths, and then use the
`get_ratios()` helper function to get the ratio with any flags. In the
case below, we don't have absorbance at 220 nm, so we get flags of
DATA01
...
`format_index()` function to make sure everything is correctly
formatted. We'll use `base::do.call()` to bind all the lists of indices
together.
...
we'll use the `get_abs_slope()` function.
...
return the results as a list using another `base::do.call()`. Since we
don't have any EEMs data we'll just make it NA.
