...
1.  **The function needs to have the following arguments:**
    ...
    d.  `mdl_dir`: the file path to the mdl files generated with \link[eemanalyzeR]{get_mdl}
...
```{r}
eemlist <- example_processed_eems
get_fluorescence(eemlist, chen_regions[[1]]$ex, chen_regions[[1]]$em, stat="max")
...
results <- lapply(names(chen_regions), function(region_name) {
  ...
  get_fluorescence(eemlist, region$ex, region$em, stat = "max")
  })
...
results <- lapply(names(chen_regions), function(region_name){
  ...
  #get flags
    missflags <- flag_missing(eemlist, ex=region$ex, em=region$em, all=FALSE)
    mdlflags <- check_eem_mdl(eemlist, eem_mdl, region$ex, region$em)
...
```{r}
chen2003 <- function(eemlist, abslist, cuvle=1, mdl_dir){
...
```{r}
indices <- get_indices(example_processed_eems, example_processed_abs, index_method = chen2003, return = "wide", 
                       mdl_dir = file.path(system.file("extdata", package = "eemanalyzeR")))
