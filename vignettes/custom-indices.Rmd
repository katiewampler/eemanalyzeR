...
To maximum flexibility in calculating indices, we've enabled the [get_indices](../reference/get-indices.html) function to accept custom written functions, allowing users to create their own function and calculate indices that may not be included in existing methods.
...
1.  **The function needs to have the following arguments:**
    ...
    d.  `qaqc_dir`: the file path to the mdl files generated with \link[eemanalyzeR]{create_mdl}
...
To get the absorbance values at a certain wavelength the [get_absorbance](../reference/get-absorbance.html) function can be used. This function can additionally provide specific absorbance (like SUVA 254).
...
A similar function exists for getting fluorescence values from EEMs data: [get_fluorescence](../reference/get-fluorescence.html). Here, you can specify a range of excitation and/or emission wavelengths. If stat is set to `sum` the sum of fluorescence will be returned, otherwise if `max` is used the maximum fluorescence across that range will be returned.
...
Lastly, we have a function for getting spectral slopes: [get_abs_slope](../reference/get-abs-slope.html) which will convert the absorbance to absorption, interpolate the data if needed, and return the spectral slope within a wavelength range.
...
First is the [get_ratios](../reference/get-ratios.html) function. This can be helpful when calculating things like peak ratios, where a missing value will result in an `NA` or a value of 0 in the denominator will result in an infinite value.
...
There's also a function that will flag indices that can't be calculated because data didn't exist for the requested wavelengths: [flag_missing](../reference/flag-missing.html). This is useful to check when calculating indices to flag why certain indices might be returning `NA` values.
...
```
You may also want to flag values below the method detection limit (MDL) this can be done with: [check_*_mdl](../reference/check_mdl.html). This is useful to ensure that the blanks are actually blank and ratio values that are returned are based on "real" data. 
...
```
If your function generates more than one flag (say a flag for missing data and an MDL flag) you can use the [.combine_flags](../reference/dot-combine-flags.html) function to nicely combine the flags. 
...
```
Once we've gotten the values and the flags, the data needs to be formatted in a specific way to input into the [get_indices](../reference/get-indices.html) function. To do this we can use the [format_index](../reference/format-index.html) function.
...
We can use the [get_fluorescence](../reference/get-fluorescence.html) helper function to calculate the fluorescence for the numerator and denominator of the ratio. Then we can use the [get_ratios](../reference/get-ratios.html) helper function to get the ratio. For a single index this would look like:
...
Now let's test our custom function with the [get_indices](../reference/get-indices.html) function. Note that we get a warning about the indices because the EEMs data hasn't gone through any processing steps.
...
We can use the [get_absorbance](../reference/get-absorbance.html) helper function to calculate the absorbance at 254 and 420. For a single wavelength that looks like:
...
Next lets get the ratios, we can still use the [get_absorbance](../reference/get-absorbance.html) helper function to get the absorbance at the wavelengths, and then use the [get_ratios](../reference/get-ratios.html) helper function to get the ratio with any flags. In the case below, we don't have absorbance at 220 nm, so we get flags of DATA01
...
Lastly we need to get the spectral slopes and spectral ratio. For these we'll use the [get_abs_slope](../reference/get-abs-slope.html) function.
...
Now let's test our custom function with the [get_indices](../reference/get-indices.html) function. Once again note that we get a warning about the indices because the EEMs data hasn't gone through any processing steps.
