...
To go from raw data to processed and clean data use the `run_eems()`
...
to consistently modify the defaults used in the `run_eems()` function
you can use the `edit_user_config()` function. This will create a YAML
file with your selected defaults which is stored on your computer will
be read when you load the package.
...
you can use the `create_mdl()` and `create_std()` functions to create
your QA/QC files. These functions require a directory of analytical
blanks and check standards (for instance the [tea
...
[`creating-multiple-methods`](creating-multiple-methods.html).
...
unlink(file.path(tempdir(), "eemanalyzeR-example"),recursive = TRUE)
```
