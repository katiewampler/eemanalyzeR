...
-   [`staRdom`](https://cran.r-project.org/web/packages/staRdom/index.html)

However, these tools require a fair bit of coding knowledge to set up a
processing procedure. The `eemanalyzeR` package aims to be simple enough that
undergraduates, who are often the ones running the instruments, can
...
To go from raw data to processed and clean data use the [run_eems](../html/run_eems.html)
...
```{r, eval=FALSE}
  run_eem(system.file("extdata", package = "eemanalyzeR"), "eemanalyzeR-example")
```
...
tree_output <- tree_output[order(file_ext(tree_output))]
...
to consistently modify the defaults used in the [run_eems](../html/run_eems.html) function you
can use the [] function. This will create a [] file stored on your
computer which will be read when you load the package.

# Creating QA/QC Standards

One of the goals with the `eemanalyzeR` package was to build in QA/QC checks to ensure data quality. While not required to use the package, you can use the [create_mdl](../html/create_mdl.html) and [create_std](../html/create_std.html) functions. These function requires a directory of analytical blanks and check standards (for instance the [tea standard](http://pubs.er.usgs.gov/publication/ofr20181096) recommended by the USGS). While you can create these standards with any number of samples, its recommended to use at least 20 for stability. 
...
#generate mdl file
eem_std <- create_std(dir, type = "eem") 
abs_std <- create_std(dir, type = "abs")
```

These files will be used when calculating absorbance and fluorescence indices to ensure: 

1. Sample index values are "real" and distinguishable above a blank (above MDL)
...
3. Check standard index values in the sample run are consistent with a long-term standard (within 20 % of mean standard)
